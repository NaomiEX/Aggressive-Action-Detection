...
class HOIAEvaluator():
    def __init__(self, preds, gts, args):
        self.correct_mat = np.load(os.path.join(args.root_path, "annotations", "corre_hoia.npy"))
...
if __name__ == "__main__":
    ...
    gts = torch.load('./stored/gts_epoch_0')[-10:]
    preds = torch.load('./stored/preds_epoch_0')[-10:]
    ...
    class Args:
        def __init__(self):
            self.use_nms = False
            self.nms_thresh = 0.5
            self.root_path = os.path.join("data", "hoia")
