...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries):
        ...
        invalid_obj_ids = [12, 26, 29, 30, 45, 66, 68, 69, 71, 83]
        self._valid_obj_ids = [i for i in range(1, 91) if i not in invalid_obj_ids]
...
def build(image_set, args):
    ...
    dataset = HICODetection(image_set, img_folder, anno_file, make_hico_transforms(image_set),
                            args.det_token_num)
    if image_set == 'val':
...
if __name__ == "__main__":
    class args():
        root_path = "data/hico_20160224_det"
        det_token_num = 100
        inter_token_num = 100
    build("train", args())
